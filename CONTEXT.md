...
**Default Rule Preset**:
The starter rule policy generated by the configuration template. It treats layer leaks and cross-context internal imports as errors, external packages as default-deny in sensitive layers, and unresolved imports, circular dependencies, and unclassified files as warnings.
...
A relative or configured-alias import that OnionCry cannot map to a project file using MVP local import resolution. External package existence is outside this rule's MVP scope.
...
**Circular Dependency**:
A cycle of local import edges between analyzed project files. Circular dependency detection applies to all resolved local files in the file universe and starts as a warning in the default preset.
_Avoid_: only cross-layer cycle
